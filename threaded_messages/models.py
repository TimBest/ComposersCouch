...
class MessageManager(models.Manager):
    ...
    def inbox_for(self, user, read=None, only_unreplied=None):
        ...
        inbox = self.filter(
            ...
            thread__creator=user,
            thread__replied=False,
        )
        if read != None:
            if read == True:
...
class Message(models.Model):
    ...
    sender = models.ForeignKey(User, related_name='messages_sent', blank=True, null=True, verbose_name=_("sender"))
...
class Thread(models.Model):
    ...
    def get_absolute_url(self):
        if hasattr(self, 'request'):
            return ('request_detail', [self.id])
        elif hasattr(self, 'show'):
            return ('show_message', [self.id])
...
class Participant(models.Model):
    ...
    user = models.ForeignKey(User, related_name='threads', verbose_name=_("participant users"), blank=True)
