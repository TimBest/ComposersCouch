...
class IsParticipantDecoratorTests(TestCase):
    ...
    def test_anonymous_user(self):
        ...
        response = mock_fn(self.request, kwargs={'thread_id':1})
    ...
    def test_non_participant(self):
        ...
        self.assertRaises(PermissionDenied, mock_fn, self.request, kwargs={'thread_id':1})
    ...
    def test_participant(self):
        for pk in [2,3]:
            ...
            user.profile.save()
            self.request.user = user
            response = mock_fn(self.request, thread_id=1)
