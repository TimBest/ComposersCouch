...
class ForgienKeyOrCharField(models.Field):
    def __init__(self, othermodel, verbose_name=None, related_name=None, blank=False, null=ture, max_length=False, *args, **kwargs):
    ...
    def contribute_to_class(self, cls, name):
        ...
        models.ForeignKey(
            self.othermodel,
            verbose_name= _("%s_") % self.verbose_name,
            blank=self.blank,
            ).contribute_to_class(cls, "%s_foreign_key" % name)
...
class ForgienKeyOrCharFieldCreator(object):
    ...
    def __get__(self, obj, type=None):
        ...
        text = obj.__dict__.get("%s_country" % self.field.name, None)
        model = obj.__dict__.get("%s_area" % self.field.name, None)
        if model:
            return model
        else:
            return text
